    ...
    <section class="py-24 bg-slate-900 overflow-hidden">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div v-for="(photo, index) in instagramPhotos.slice(0, 8)" :key="index" class="aspect-square bg-slate-800 rounded-2xl overflow-hidden relative group cursor-pointer shadow-2xl">
            ...
            <img :src="photo" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
          </div>
          
          <!-- Mensagem caso não tenha fotos -->
          <div v-if="instagramPhotos.length === 0" v-for="i in 4" :key="i" class="aspect-square bg-slate-800 rounded-2xl flex items-center justify-center p-6 text-center">
            <span class="text-slate-500 text-xs italic">Adicione fotos em assets/instagram</span>
          </div>
        </div>
      </div>
    </section>
