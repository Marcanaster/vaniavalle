          ...
          <div class="space-y-2">
            ...
            <div v-for="matricula in turmaSelecionada?.alunosMatriculados?.filter(m => m.ativo)" :key="matricula.id" class="flex justify-between items-center p-3 hover:bg-slate-50 border border-slate-100 rounded-lg group transition-all">
              ...
              <div class="flex items-center gap-2">
                <button @click="regerarFatura(matricula.alunoId)" class="text-slate-400 hover:text-indigo-600 transition-colors p-1" title="Regerar Fatura de Matrícula">
...
const regerarFatura = async (alunoId) => {
  try {
    await api.post(`/turmas/${turmaSelecionada.value.id}/regerar-fatura/${alunoId}`)
    toast.success('Fatura regerada com as novas condições!')
  } catch (error) {
    toast.error('Erro ao regerar fatura')
  }
}
</script>
